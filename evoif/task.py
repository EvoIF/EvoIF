...
class ResidueTypePrediction(tasks.AttributeMasking, core.Configurable):
    ...
    def predict_and_target(self, batch, all_loss=None, metric=None):
        ...
        if self.graph_construction_model:
            try:
                ...
                print(f"Graph construction model failed: {e}")
                print(f"Batch: {batch}")
        ...
        # Mask struc_profile and if_profile at the same positions as graph residues
        mask_mask = (replace_prob >= 0.2)  # 80% of positions that are actually masked
        masked_node_index = node_index[mask_mask]
        for field_name in ["if_profile","struc_profile"]:
            if field_name not in batch:
                continue
            if field_name.endswith("profile"):
                batch[field_name][masked_node_index] = 0 
            else:
                batch[field_name][masked_node_index] = 21
        
        if self.graph_construction_model:
           graph = self.graph_construction_model.apply_edge_layer(graph)
        input = graph.residue_feature.float()
        batch.pop("graph")
        output = self.model(graph, input, all_loss, metric, batch=batch)
        node_feature = output["node_feature"][node_index]
        node_feature = self.dropout(node_feature)
        pred = self.linear(node_feature)+output['profile_logits'][node_index]
    ...
    def inference(self, batch):
        ...
        input = graph.residue_feature.float()
        output = self.model(graph, input, batch=batch)
        node_feature = output["node_feature"]
        pred = self.linear(node_feature)+output['profile_logits']
        if self.plddt_threshold:
            # for AlphaFold2-predicted structure pdb files, plddt is saved as b_factor
            plddt_mask = graph.b_factor < self.plddt_threshold
